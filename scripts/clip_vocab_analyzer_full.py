#!/usr/bin/env python3
"""
CLIP-based Vocabulary Image Analyzer - Full 170 Images
...
class CLIPVocabAnalyzer:
    ...
    def __init__(self, model_name="ViT-B/32", vocab_file="vocab/vocab_list.txt"):
        ...
        print(f"🚀 Initializing CLIP Vocabulary Analyzer (Full Dataset)")
    ...
    def analyze_vocabulary_dataset(self, start_id: int = 4, end_id: int = 173) -> List[Dict]:
        """Analyze complete vocabulary dataset using CLIP"""
        print(f"🚀 Analyzing COMPLETE vocabulary dataset with CLIP")
        print(f"📊 Processing vocab-{start_id:03d} to vocab-{end_id:03d} ({end_id-start_id+1} images)")
        ...
        for i in range(start_id, end_id + 1):
            ...
            print(f"\n📸 Processing vocab-{screenshot_id}.png (expected: {expected_vocab}) [{i-start_id+1}/{end_id-start_id+1}]")
            ...
            if (i - start_id + 1) % 10 == 0:
                ...
                processed = i - start_id + 1
                remaining = end_id - i
                eta = (elapsed / processed) * remaining
                print(f"   📊 Progress: {processed}/{end_id-start_id+1} ({processed/(end_id-start_id+1)*100:.1f}%) - ETA: {eta/60:.1f}min")
        ...
        print(f"   Processing time: {total_time:.2f}s ({total_time/60:.1f}min)")
        ...
        output_file = f"clip_vocab_analysis_full_{int(time.time())}.json"
...
def main():
    """Main function to run CLIP analysis on all 170 images"""
    print("🚀 CLIP Vocabulary Analysis - Full Dataset")
    print("=" * 60)
    ...
    # Analyze all 170 images
    print("\n🔍 Analyzing ALL 170 vocabulary images with CLIP...")
    results = analyzer.analyze_vocabulary_dataset(start_id=4, end_id=173)
    ...
    print(f"\n🎯 FINAL SUMMARY:")
    print(f"   📊 Images processed: {len(successful_results)}/170")
    print(f"   ✅ Terms found: {len(found_results)}")
    print(f"   📈 Success rate: {len(found_results)/len(successful_results)*100:.1f}%")
